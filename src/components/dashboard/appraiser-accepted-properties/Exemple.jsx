...
export default function Exemple({
  ...
  useEffect(() => {
    ...
    axios
      ...
      .then((res) => {
        const { success, data: bidsData, message } = res?.data;
        if (success) {
    ...
    axios
      ...
      .then((res) => {
        ...
        if (success) {
          ...
          setAllBrokers(allbroker); // temporarily set brokers until brokerage loaded
        }
      })
      .catch(() => {
        setErrorMessage("Unexpected error occurred.");
    ...
    // Fetch brokerage companies separately for easier debug
    axios
      ...
      .then((res) => {
        const { success, data: brokerageData, message } = res?.data;
        if (success) {
          const allbrokerage = brokerageData?.result?.$values || [];
          setAllBrokers((prev) => [...prev, ...allbrokerage]);
        }
      })
      .catch(() => {
        setErrorMessage("Unexpected error occurred.");
