  ...
  useEffect(() => {
    ...
    axios
      .get("/api/getAllBids", {
        ...
        validateStatus: () => true, // ✅ let us handle status manually
      })
      .then((res) => {
        ...
        if (success) {
          ...
          toast.error(res.data?.message || "Failed to fetch bids");
          setModalIsOpenError(true);
        }
      })
      .catch((err) => {
        console.error("Axios getAllBids error:", err);
        toast.error("Network error or unexpected failure");
        setModalIsOpenError(true);
      })
      .finally(() => {
        toast.dismiss(); // dismiss any loader/spinner toast
      });
